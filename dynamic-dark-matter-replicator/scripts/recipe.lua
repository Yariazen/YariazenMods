...
function getRecipes(item)
	...
	for i, recipe in pairs(data.raw.recipe) do
		if i ~= "coal-liquefaction" and
			...
			local products = getProducts(recipe.prototype)
	...
	for name, amount in pairs(getIngredients(recipe)) do
		...
		if (#subrecipes == 0) then
			...
			raw_ingredients["time"] = raw_ingredients["time"] +
				(best_recipe.recipe.energy or 1) / best_machine[best_recipe.recipe.category].speed * amount / multiple
			raw_ingredients["energy"] = raw_ingredients["energy"] +
				(best_recipe.recipe.energy or 1) / best_machine[best_recipe.recipe.category].speed *
				best_machine[best_recipe.recipe.category].energy * amount / multiple
